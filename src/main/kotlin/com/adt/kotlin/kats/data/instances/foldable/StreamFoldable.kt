...
interface StreamFoldable : Foldable<StreamProxy> {
     ...
     */
    override fun <A, B> foldLeft(v: StreamOf<A>, e: B, f: (B) -> (A) -> B): B = v.narrow().foldLeft(e, f)

    override fun <A, B> foldLeft(v: StreamOf<A>, e: B, f: (B, A) -> B): B = foldLeft(v, e, C2(f))
     ...
     * @return                  folded result
     */
    override fun <A, B> foldRight(v: StreamOf<A>, e: B, f: (A) -> (B) -> B): B = v.narrow().foldRight(e, f)

    override fun <A, B> foldRight(v: StreamOf<A>, e: B, f: (A, B) -> B): B = foldRight(v, e, C2(f))
