...
import com.adt.kotlin.kats.data.instances.monadplus.ListMonadPlus
import com.adt.kotlin.kats.data.instances.monoid.ListMonoid
import com.adt.kotlin.kats.data.instances.semigroup.ListSemigroup
import com.adt.kotlin.kats.data.instances.traversable.ListTraversable
...
typealias ListOf<A> = Kind1<ListProxy, A>

sealed class List<out A> : Kind1<ListProxy, A>, Iterable<A> {

    class ListProxy private constructor()             // proxy for the List context
     ...
     *
     *   [].ap(functions) == []
     *   [1, 2, 3, 4].ap(functions) == [0, 0, 0, 0, 101, 102, 103, 104, 1, 4, 9, 16]
     *   [1, 2, 3, 4].ap([]) == []
     */
    fun <B> ap(f: List<(A) -> B>): List<B> {
        tailrec
    ...
    fun <B> fmap(f: (A) -> B): List<B> = this.map(f)
     ...
     * Examples:
     *   [1, 2, 1, 2, 3].nub = [1, 2, 3]
     *   [1, 1, 3, 2, 1, 3, 2, 4].nub = [1, 3, 2, 4]
     *   [].nub = []
     *
     * @return                  new list with all duplicates removed
     */
    fun nub(): List<A> = this.removeDuplicates()
     ...
     *
     * Examples:
     *   [2, 3, 5, 7, 8].nubBy{x -> {y -> (x + y == 10)}} = [2, 3, 5]
     *   [8, 7, 5, 3, 2].nubBy{x -> {y -> (x + y == 10)}} = [8, 7, 5]
     *
     * @param eq                user supplied equality predicate
     * @return                  xxx
     */
    fun nubBy(eq: (A) -> (A) -> Boolean): List<A> =
        when (this) {
            is Nil -> Nil
            is Cons -> {
                val hd: A = this.head()
                val tl: List<A> = this.tail()
                Cons(hd, tl.filter{a: A -> !eq(hd)(a)}.nubBy(eq))
            }
        }   // nubBy

    fun nubBy(eq: (A, A) -> Boolean): List<A> = nubBy(C2(eq))
     ...
     *   [1, 2, 3, 4].rotateLeft(4) == [1, 2, 3, 4]
     *   [1, 2, 3, 4].rotateLeft(6) == [3, 4, 1, 2]
     *   [].rotateLeft(3) == []
     *
     ...
     *   [1, 2, 3, 4].rotateRight(4) == [1, 2, 3, 4]
     *   [1, 2, 3, 4].rotateRight(6) == [3, 4, 1, 2]
     *   [].rotateRight(3) == []
     *
     ...
     */
    fun <G, B> traverse(ag: Applicative<G>, f: (A) -> Kind1<G, B>): Kind1<G, List<B>> {
        val self: List<A> = this
        return ag.run {
            val consF: (A) -> (Kind1<G, List<B>>) -> Kind1<G, List<B>> =
                    {a: A ->
                        {xs: Kind1<G, List<B>> ->
                            ag.run{
                                {kgb: Kind1<G, B> ->
                                    {kglb: Kind1<G, List<B>> ->
                                        ap(kglb, fmap(kgb){b: B -> {bs: List<B> -> ListF.cons(b, bs) }})
                                    }
                                }
                            }(f(a))(xs)
                        }
                    }
            self.foldRight(pure(ListF.nil()), consF)
        }
    }   // traverse
    ...
    companion object {
         ...
         */
        fun <A> semigroup(): Semigroup<List<A>> = ListSemigroup()

        /**
         * Create an instance of this monoid.
         */
        fun <A> monoid(): Monoid<List<A>> = ListMonoid()

        /**
         * Create an instance of this functor.
         */
        fun functor(): Functor<ListProxy> = object: ListFunctor {}

        /**
         * Create an instance of this applicative.
         */
        fun applicative(): Applicative<ListProxy> = object: ListApplicative {}

        /**
         * Create an instance of this monad.
         */
        fun monad(): Monad<ListProxy> = object: ListMonad {}

        /**
         * Create an instance of this foldable.
         */
        fun foldable(): Foldable<ListProxy> = object: ListFoldable {}

        /**
         * Create an instance of this traversable.
         */
        fun traversable(): Traversable<ListProxy> = object: ListTraversable {}

        /**
         * Create an instance of this alternative.
         */
        fun alternative(): Alternative<ListProxy> = object: ListAlternative {}

        /**
         * Create an instance of this monadplus.
         */
        fun monadPlus(): MonadPlus<ListProxy> = object: ListMonadPlus {}

        /**
         * Entry point for monad bindings which enables for comprehension.
         */
        fun <A> forC(block: suspend MonadSyntax<ListProxy>.() -> A): List<A> =
                List.monad().forC.monad(block).narrow()
